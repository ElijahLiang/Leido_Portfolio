...
const CardSwap = forwardRef(({
  ...
  const runSwap = (targetIdx) => {
    if (order.current.length < 2) return;

    const currentFront = order.current[0];
    if (targetIdx !== undefined && currentFront === targetIdx) return;
    ...
    if (targetIdx !== undefined) {
      ...
      refs.forEach((r, i) => {
        const idx = order.current.indexOf(i);
        placeNow(r.current, makeSlot(idx, cardDistance, verticalDistance, refs.length), skewAmount);
      });
      return;
    }
