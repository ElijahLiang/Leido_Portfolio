...
function makeBuffer(gl, data, usage) {
  ...
  gl.bufferData(gl.ARRAY_BUFFER, data, usage); gl.bindBuffer(gl.ARRAY_BUFFER, null); return b;
}

function createTexture(gl, min, mag, ws, wt) {
...
class ArcballControl {
  ...
  constructor(canvas, cb) {
    ...
    canvas.addEventListener('pointerdown', e => {
      ...
      vec2.copy(this.previousPointerPos, this.pointerPos); this.isPointerDown = true;
    });
...
class InfiniteGridMenu {
  ...
  constructor(canvas, items, onActiveItemChange, onMovementChange, onInit = null, scale = 1.0) {
    ...
    this.camera.position[2] = 3 * scale; this.#init(onInit);
  }
  ...
  #initTexture() {
    ...
    Promise.all(this.items.map(item => new Promise(resolve => {
      ...
      images.forEach((img, i) => {
        if (!img) return;
        const cx = (i % this.atlasSize) * cellSize, cy = Math.floor(i / this.atlasSize) * cellSize;
        // cover-fit: crop to fill square cell
        const imgRatio = img.width / img.height;
        let sx = 0, sy = 0, sw = img.width, sh = img.height;
        if (imgRatio > 1) { sx = (img.width - img.height) / 2; sw = img.height; }
        else { sy = (img.height - img.width) / 2; sh = img.width; }
        ctx.drawImage(img, sx, sy, sw, sh, cx, cy, cellSize, cellSize);
      });
  ...
  #initDiscInstances(count) {
    ...
    for (let i = 0; i < count; ++i) {
      const a = new Float32Array(this.discInstances.matricesArray.buffer, i * 16 * 4, 16);
      a.set(mat4.create()); this.discInstances.matrices.push(a);
    }
  ...
  #onControlUpdate(dt) {
    ...
    if (!this.control.isPointerDown) {
      ...
      ctz += this.control.rotationVelocity * 80 + 2.5; damping = 7 / ts;
    }
